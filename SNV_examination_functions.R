...
identify_snv_sets_positions <- function(snv_positions, 
  ...
  gen_common_snvs <- data.frame(Position = gen_common_positions, 
                                File = "gen common")
  ...
  nas_common_positions <- Reduce(intersect, 
                                 lapply(nas_indices, 
                                        function(i) snv_positions[[i]]))
  nas_common_snvs <- data.frame(Position = nas_common_positions, 
                                File = "nas common")
  
  # Identify common SNVs across all files
  all_common_positions <- Reduce(intersect, snv_positions)
  all_common_snvs <- data.frame(Position = all_common_positions, 
                                File = "all common")
  
  # Combine all sets
  combined_positions <- bind_rows(unique_snvs, 
                                  gen_common_snvs, 
                                  nas_common_snvs, 
                                  all_common_snvs)
